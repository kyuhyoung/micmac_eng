...
// ---------------------------------------------------------
// Algorithme de correction du retard troposphrique
// Modle de Takeyasu SAKAI (GPS programming)
// ---------------------------------------------------------
double AtmosphericModel::tropo_correction(ECEFCoords usr, ECEFCoords sat) {

    // Elvation du satellite
    double el = usr.elevationTo(sat);

    // Altitude du rcepteur
    double h = usr.toGeoCoords().height;

    // computation de la correction
    double d = 1.0;
...
// ---------------------------------------------------------
// Algorithme de correction du retard troposphrique
// Utilisation du modle de Saastomonien
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// Algorithme de correction du retard ionosphrique with le
// modle de Klobuchar au znith
// ---------------------------------------------------------
double AtmosphericModel::iono_correction_raw(ECEFCoords usr) {

    // Azimut/lvation du satellite
    double az = 0;
    double el = PI/2;
    double el_sc = el/PI;

    // Coordonnes godsiques du rcepteur
    GeoCoords geo_usr = usr.toGeoCoords();

    // Coordonnes du point d'between iono
    ...
    // Variable polynomiale du modle
    double x = (2*PI*(tl-14*3600))/per;
    while (x > +PI) x = x - 2*PI;
    while (x < -PI) x = x + 2*PI;

    // computation du retard
