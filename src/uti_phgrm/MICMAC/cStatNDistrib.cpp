...
REAL cStatOneClassEquiv::Cov() const
{
    ...
    if (verbose)
    {
      ...
      std::cout << "mNbOK : "<<mNbOK<<std::endl; //Nb de pixel in la vignette
      std::cout << "mKCurDist : "<<mKCurDist<<std::endl; //Nb d'images dispo
    }

    REAL aSigmaTot = 0.0;

     for (int aIndPix=0; aIndPix<mNbOK ; aIndPix++)// for chaque pixel de la vignette
     {
         int aKPix = mIndOK[aIndPix];// Le pointeur in le array for ce pixel
         {
            REAL aS1=0.0;
            REAL aS2=0.0;
            for (INT aKDist=0; aKDist<mKCurDist ; aKDist++)// for toutes les images dispo
            {
...
//       cout<<"histo2d["<<i<<"]["<<j<<"] "<<histo2d[i][j]<<"\n";
  ...
  // au nb de pixels in la fen�tre : (2N+1)*(2N+1) (N = taille de la 1/2 fenetre)
