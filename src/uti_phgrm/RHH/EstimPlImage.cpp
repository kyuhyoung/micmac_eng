...
// H1on2 permet d'inverser les roles de H1 and H2 ("or" se fait la comparaison) for tester la dissymetrie

// TestDiff if active permet de verifier les equation en refaisant le computation a la main
...
std::string cImagH::EstimatePlan()
{
    ...
    // On regarde if on est en mode focus on une une image (mise au point);
     mPlanEst = false;
     ...
     */
     ...
     for (int aK=0 ; aK<int(VLink().size()) ; aK++)
     {
      ...
      int aKBEst = -1;
       ...
       double aMulXZ =  aAltiCible / aCentre0.z; // if <0 remet les camera par dessus
       double aMulY  =  ElAbs(aMulXZ);           // for que la transfo soit directe
