...
#include "StdAfx.h"
//for utiliser MedianSup on un vector
...
{
public:
    // constructeur du modèle line 2D à partir de 2 points
    c2DLineModel();
    c2DLineModel(Pt2dr aP1, Pt2dr aP2);
    // je construit l'object en connaissant a and b
    c2DLineModel(double a, double b);
    // à partir d'un point, a=0
    c2DLineModel(Pt2dr aP1);
    // on donne une list d'observations and éventuellement un pourcentage d'outliers à baquer and on demande de compute la distance à la droite
...
public:
    cRansac_2dline();
    // création de l'object à partir des observations, définition de la forme du modèle, d'un vector de pondération
...
private:
    ...
    int mNbIt, mPrcOutliers,mNbItConvergence; // number d'itération, pourcentage d'outliers qu'on jette lors du computation du coût.
    std::vector<Pt2dr> * mObs;
    std::vector<double> mPond;
    bool mQuiet;

    c2DLineModel oneIteration(std::vector<Pt2dr> * aObs); // compute le cout de myCurrentModel on les observation
...
public:
    cLSQ_2dline();
    // création de l'object à partir des observations, définition de la forme du modèle
